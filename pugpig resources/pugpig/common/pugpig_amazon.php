...
function _amazon_build_endpoint_url($cmd, $user_id, $token, $url, $amazonSecret) {
  if (!isset($url) || $url == '') {
...
function _amazon_renew_token($user_id, $product_sku, $subs_sku, $token, $base_url, $amazonSecret, $proxy_server='', $proxy_port='') {
  $url = _amazon_build_endpoint_url('renew', $user_id, $token, $base_url, $amazonSecret);
...
function _amazon_verify_token($user_id, $product_sku, $subs_sku, $token, $base_url, $amazonSecret, $proxy_server='', $proxy_port='') {
  // see https://developer.amazon.com/sdk/in-app-purchasing/documentation/rvs.html
  $comments = array();

  $url = _amazon_build_endpoint_url('verify', $user_id, $token, $base_url, $amazonSecret);
  ...
  switch ($http_code) {
    case 400:
      ...
      break;
    case 499:
      $token = _amazon_renew_token($user_id, $product_sku, $subs_sku, $token, $base_url, $amazonSecret);
      if (isset($token) && strlen($token) > 0) {
        return _amazon_verify_token($user_id, $product_sku, $subs_sku, $token, $base_url, $amazonSecret);
      }
      break;
    case 200:
      $status = 'SUBSCRIPTIONEXPIRED';
      ...
      if ($itemType == 'ENTITLED' && $sku == $product_sku)
        $status = 'OK';
      else if ($itemType == 'SUBSCRIPTION' && $sku == $subs_sku) {

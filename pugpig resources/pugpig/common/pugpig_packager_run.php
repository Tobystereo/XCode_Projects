...
include 'pugpig_interface.php';
include 'pugpig_http.php';
include 'pugpig_manifests.php';
include 'pugpig_packager.php';
include 'http_build_url/http_build_url.php';
...
Use this function to ensure the top level OPDS end points are working as expected.
<form method="get">
<input type="hidden" name="action" value="checkendpoints" />
<table border="0">
<tr><td><label for="f">Top Level Feeds</label></td><td> <textarea name="f" value="" rows="4" cols="90" /><?php echo $entrypoints; ?></textarea></td></tr>
...
<table border="0">
<tr><td><label for="p">Edition Package feed</label></td><td> <input type="text" name="p" value="" size="120" /></td></tr>
<tr><td><label for="c">Edition ATOM feeds</label></td><td> <input type="text" name="c" value="" size="120" /></td></tr>
...
<tr><td><label for="debug">Debug Mode</label></td><td> <input type="checkbox" name="debug" checked /></td></tr>
...
<?php
  ...
  if ($action == 'generatepackagefiles') {
    ...
    $package_xml = _pugpig_package_edition_package(
      ...
      $tmp_root, $save_root, $cdn, $package_url_base, $test_mode, $image_test_mode);
  } else  if ($action == 'checkendpoints') {
    $feeds = $_GET['f'];
    $endpoints = explode("\r\n", $feeds);

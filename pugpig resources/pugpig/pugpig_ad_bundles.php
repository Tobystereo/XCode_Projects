...
************************************************************************/
add_action('init', 'pugpig_ad_bundles_register');

function pugpig_ad_bundles_register() {

  $labels = array(
    'name' => _x('Ad Bundles', 'post type general name'),
    'singular_name' => _x('Ad Bundle', 'post type singular name'),  
    'add_new' => _x('Add New', 'Ad Bundle item'),
    'add_new_item' => __('Add New Pugpig Ad Bundle'),
    'edit_item' => __('Edit Ad Bundle Item'),
    'new_item' => __('New Ad Bundle Item'),
    'view_item' => __('View Ad Bundle Item'),
    'search_items' => __('Search Ad Bundles'),
...
function pugpig_ad_bundle_init(){
  add_meta_box("pugpig-ad_bundle_info-meta", "Ad Bundle Info", "pugpig_ad_bundle_info", PUGPIG_AD_BUNDLE_POST_TYPE, "normal", "high");  
}
...
function pugpig_ad_bundle_html_file($unzipped_path) {
  ...
  if ($index == '') {
    ...
    if ($index != '') {
      ...
      pugpig_add_admin_notice('Ad Packager: Found index: ' . $index, "error");
    }
...
add_filter('pugpig_flatplan_style', 'spectator_pugpig_flatplan_style', 10, 2);
function spectator_pugpig_flatplan_style($output, $post) {
  if ($post->post_type != PUGPIG_AD_BUNDLE_POST_TYPE) return $output;
...
function pugpig_ad_bundle_url($post) {
  ...
  return pugpig_ad_bundle_get_unzip_url($post->ID) . $html_file;
}
